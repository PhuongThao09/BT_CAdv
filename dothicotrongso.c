...
void addEdge(Graph graph, int v1, int v2, double weight);//add canh co huong tu v1 toi v2
...
int main(){
  ...
  if(w = INFINITIVE_VALUE){
    ...
    printf("duong di ngan nhat tu dinh %s den dinh %s co do dai la %f\n", getVertex(g, s), getVertex(g, t), shortestPath(g, s, t, path, &length));
    for(i = 0;i<length;i++)
...
double shortestPath(Graph g, int s, int t, int* path, int*length){
  ...
  while(!dll_empty(queue)){
    //get u from the priority queue lay u tu hang doi dau tien
